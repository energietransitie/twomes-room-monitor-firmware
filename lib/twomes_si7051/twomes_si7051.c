...
uint16_t read_si7051() {
  ...
  esp_err_t err = (twomes_i2c_read(SI7051_I2C_ADDRESS, i2c_data, sizeof(i2c_data)));
...
uint16_t read_si7051_with_retries(uint8_t attempts) {
  ...
  if (err != ESP_OK) {
    attempts--;
    ESP_LOGD("TEMPERATURE", "Reading temperature failed. retrying %u more times", attempts);
    if (attempts == 0) {
      ESP_LOGE("TEMPERATURE", "Reading SI7051 failed!");
      return 0;
    }
    read_si7051_with_retries(attempts);
  }
